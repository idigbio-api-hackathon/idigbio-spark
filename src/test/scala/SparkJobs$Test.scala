...
class SparkJobs$Test extends FlatSpec with BeforeAndAfterAll with Matchers {
  ...
  "list of traits for taxon" should "be filtered using trait filter" in {
    ...
    val actualTraitFilter: Map[String, String] = TraitFilter.parseTraitConfig( """bodyMass greaterThan 1025 g""")

    val rdd = sc.parallelize(fourLines)
    val rows = ChecklistGenerator.readRows(header, rdd)

    val selectedNamesByTraitsRDD = rows
      .filter(record => TraitFilter.hasTrait(actualTraitFilter, record.toMap))
      .map(record => {
      record.find(_._1 == "Scientific Name") match {
        case Some((_, aName)) => aName.trim
        case _ => ""
      }
    }).distinct().filter(_.nonEmpty)

    val selectedNames = selectedNamesByTraitsRDD.collect()
    selectedNames should not contain "Netuma thalassina"
    selectedNames should not contain "Sargochromis carlottae"
    selectedNames should contain("Pseudopentaceros wheeleri")
    selectedNames should contain("Balaena mysticetus")
    selectedNames should contain("Balaenoptera musculus")
  }
